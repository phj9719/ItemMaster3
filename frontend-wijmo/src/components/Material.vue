<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-title v-if="value._links">
            Material # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
        </v-card-title >
        <v-card-title v-else>
            Material
        </v-card-title >
...
export default {
    ...
    created(){
        ...
        websocketUrl.pathname = "/rsocket/materials";
        websocketUrl.hash = "";
        ...
        client.connect().subscribe({
            ...
            const endpoint = "materials."+ me.value.id +".get"
    ...
    methods: {
        ...
        async save(){
            try {
                ...
                if(!this.offline) {
                    if(this.isNew) {
                        temp = await axios.post(axios.fixUrl('/materials'), this.value)
                    } else {
